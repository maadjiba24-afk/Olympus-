...
`witness.tracked_files()` vouches for every package file regardless of suffix,
and `olympus verify` re-checks each one against the signed manifest. So a file
that `[tool.setuptools.package-data]` does not ship makes verify fail on *every
installed copy*:
...
def _declared_patterns(section_name: str = "package-data") -> list[str]:
    """The ``olympus = [...]`` globs from one setuptools data section."""
    text = (ROOT / "pyproject.toml").read_text(encoding="utf-8")
    section = re.search(
        rf"^\[tool\.setuptools\.{re.escape(section_name)}\]"
        r"(.*?)(?=^\[|\Z)",
        text, re.S | re.M)
    assert section, (
        f"pyproject has no [tool.setuptools.{section_name}] section")
...
def _shipped(rel: str, patterns: list[str]) -> bool:
    ...
    A ``.py`` file ships as a module when its directory is an importable
    package; anything else needs a package-data glob. ``fnmatch`` is only the
    per-file coverage check: separate contracts below require explicit root,
    recursive, and dotfile globs so its slash/dot semantics cannot make a
    narrow packaging declaration look complete."""
...
@pytest.mark.parametrize("rel", [
    ...
    "new/subdir/template.jinja2",
    ".package-state",
    "new/subdir/.package-state",
])
def test_future_package_data_is_shipped_regardless_of_name_or_suffix(rel):
    assert _shipped(rel, _declared_patterns()), (
        f"future signed package member {rel!r} would not ship")


def test_generated_bytecode_is_excluded_without_excluding_native_modules():
    excluded = _declared_patterns("exclude-package-data")
    for generated in ("module.pyc", "module.pyo",
                      "__pycache__/module.cpython-312.pyc",
                      "nested/__pycache__/module.cpython-312.pyc"):
        assert _shipped(generated, excluded), (
            f"generated bytecode {generated!r} is not excluded")
    for signed_native in ("module.pyd", "module.so", "module.dll"):
        assert not _shipped(signed_native, excluded), (
            f"native release input {signed_native!r} would be excluded")

...
| 8 | Two specialists can still clobber the same explicitly-named workspace file | Per-worker re-rooting was built, adversarially reviewed, and REJECTED (approved actions executed in a different root than previewed — ADR 0005 amendment 1); the safe design pins one root through prepare→approve on the actions spine. |

...
| E26 | HIGH — **FIXED in 0.27.1** | pyproject.toml (package-data) | Release integrity | **Every published wheel failed its own `olympus verify`** — the command RELEASING.md gives auditors | Installed 0.25.0/0.26.0/0.27.0 from PyPI: all print `[verify] missing tracked file: profiles/README.md`. `witness.tracked_files()` (witness.py:375) vouches for every .py/.md/.json; package-data shipped only `prompts/*.md` + top-level `*.json` | The signed manifest and the wheel were built from two different notions of "the package's files"; `olympus/profiles/README.md` matched no pattern | The advertised "verify it yourself" integrity story was broken for every PyPI user, across at least 3 releases | Make the globs express the tracked-files rule (done in 0.27.1) + a signed-⊆-shipped test |
| E27 | **CRITICAL — FIXED** | web.py:2050 `_principal` / web.py:38 `_user_for` | Cost control | **Operator-funded free chats were unlimited to any anonymous caller** — the quota identity is caller-chosen | `_principal` falls back to `_user_for(sid)` = `web-<sid>` when `accounts.require_login()` is false (the default), and sid comes from the caller's own cookie (`_resolve_sid`, web.py:183). Demonstrated: burning the allowance then changing the cookie returns a fresh one | The free-chat counter is keyed on an identity the caller mints | A keyless visitor spends the operator's API key without bound — **on a single node, no replicas needed**. This is strictly worse than the cross-replica bypass P0-1 describes | Per-IP ceiling (`config.free_chats_per_ip`, default 3x the per-user allowance) on the operator-funded path |
| E28 | HIGH — **FIXED** | web.py:2288 | Rate limiting | **`/v1/chat/completions` and `/v1/messages` had no throttle at all** | Both dialects `return` at web.py:2288-2295, before the per-IP limiter at :2331 and before `_session()` at :2339 | The /v1 dispatch was added above the guard rather than below it | The most expensive endpoint in the process (a full council run) was the only one with no rate limit | Own budget `_v1_limit()` / `OLYMPUS_V1_RATE_LIMIT`, default 120/min — looser than browser chat because /v1 is programmatic |
| E29 | HIGH — **FIXED** | web.py:2564 vs :2571 | Quota ordering | **Quota was charged before validation could still reject the request** | The comment at web.py:2561-2563 promises "a rejected request must not burn one of the user's OLYMPUS_DAILY_CHATS", but `settings.validate()` ran after the charge and can return 400 | Charge placed above validation | A malformed request cost the user a daily chat, and a free operator-funded one | Build+validate settings before charging |
| E30 | HIGH — **FIXED** | web.py:2550 / :2569 | Quota race | **Free-chat allowance was check-then-act** | `_daily_count("free:"+user)` and `_daily_bump("free:"+user)` were separate operations with the key wall between them | No atomic check-and-consume | N concurrent requests all read the same pre-charge count and every one was granted a free chat — proven by a 40-thread test | Single atomic `_daily_limited("free:"+user, free_chats())` |
| E31 | MEDIUM | web.py:2069 / metrics.py:35 | Liveness probe | `/healthz` does disk I/O despite its docstring forbidding it | web.py:2063-2066: "Must stay cheap and must NOT consult config or disk — a liveness probe that fails on a full disk causes a restart loop". It calls `metrics.snapshot()`, which calls `usage.budget_status()` + `usage.today_spend()`, both reading the usage ledger | `snapshot()` grew spend reporting after `/healthz` adopted it | An unauthenticated caller drives ledger reads on every liveness probe; contradicts its own stated constraint | Split a cheap uptime-only accessor for `/healthz` |
...
- Analysis: 14 subsystem analyzers (persistence-ops re-run after a degenerate first pass) + 40
  ...
  ~108 findings (1 CRITICAL / 25 HIGH / 39 MEDIUM / 33 LOW / 10 INFO).

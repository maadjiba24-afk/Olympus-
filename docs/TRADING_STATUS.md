...
- **Scale:** ~51,700 lines across 83 modules (48 top-level + 35 in `native/`); 66 test files; **3057 trading tests passing**
...
| `native/` | 🟡 | 35 modules, 18,914 lines, 529 tests. A 38-channel market-state schema, dataset provenance with five leakage defences, stable encoder contracts, seven representation candidates, a multi-task model (fifteen registered tasks, seven trainable), nine structural abstention reasons, a reproducible training pipeline, a stratified evaluation over eight strata, and nine capabilities behind a register whose readiness verdict is computed rather than declared. **Fitted only to synthetic series, where it loses to persistence and its intervals are 3.1–3.4× too wide; nine of nine capabilities are research-usable and zero are production-eligible** — see `docs/OLYMPUS_NATIVE_MODEL_STATUS.md` and `docs/OLYMPUS_NATIVE_CAPABILITIES.md` |

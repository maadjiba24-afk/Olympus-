...
| 2026-07-02 | **Shortlist complete — 16/16.** Candidate #10 (dashboard administration) shipped as the three-phase operator admin panel (PRs #93–#95: read-only overview → act on running state → configuration from the browser w/ vault-backed credentials and restart-honesty; see docs/ADMIN_PANEL.md). Gateway edges closed (voice notes + auto-resume on every platform, PR #96) and the v0.18 "Judgment" adoptions finished (/learn, /journey, /moa one-shot + traces, /goal wait, /prompt, /reasoning, /timestamps — PR #97). Remaining unadopted from Hermes = only the deliberate exclusions (desktop app, platform breadth, kanban UI, i18n) | Claude session (hermes-release-tracking) |
